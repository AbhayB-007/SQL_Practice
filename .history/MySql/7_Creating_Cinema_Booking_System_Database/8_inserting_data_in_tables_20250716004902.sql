...
SELECT * FROM (SELECT 'Chaplin', 72 UNION ALL
               SELECT 'Kubrick', 36 UNION ALL
               SELECT 'Coppola', 36) AS tmp
...
SELECT
    CONCAT(f.first_name, ' ', f.last_name) AS film_name,
    f.length_min,
    s.seat_row,
...
JOIN reserved_seat rs ON b.booking_id = rs.booking_id
JOIN seats s ON rs.seat_id = s.seat_id
JOIN screenings sc ON b.screening_id = sc.screening_id
JOIN films f ON sc.film_id = f.film_id
JOIN rooms r ON sc.room_id = r.room_id
ORDER BY c.customer_id, sc.start_time;